...
class SnowDepthComparator:
    ...
    def _align_rasters(self, sat_data, sat_transform, sat_crs, 
        ...
        """
        Align two rasters to the same grid for comparison
        ...
        # Handle CRS safely
        try:
            # Check if CRS values are None and handle appropriately
            if sat_crs is None:
            ...
            # For now, do basic alignment by checking dimensions
            # In a full implementation, you'd use rasterio.warp.reproject to handle CRS differences
            if sat_data.shape == gt_data.shape:
                return sat_data, gt_data
            else:
                # Simple resize for demonstration
                from scipy.ndimage import zoom
                zoom_factors = (sat_data.shape[0] / gt_data.shape[0], 
                              sat_data.shape[1] / gt_data.shape[1])
                gt_data_aligned = zoom(gt_data, zoom_factors, order=1, mode='nearest')
                return sat_data, gt_data_aligned
                
        except Exception as e:
            print(f"Warning: Error in CRS handling ({str(e)}), using simple alignment")
...
def main():
    ...
    satellite_dir = "enhanced_snow_depth_monthly_201801_202112_500m"
    ...
    comparator = SnowDepthComparator(satellite_dir, ground_truth_dir, output_dir)
